...
### 🗓️ 100 天进度表 · 游戏化成长地图
把长期目标拆成每天可完成的一小步，勾选推进、看见整体进度。**ADHD 友好：不用一次想完 100 天，只要搞定今天这一格。**

### 🥋 技巧库（Atlas）
按站立 / 防守 / 压制 / 降服等维度整理技术笔记，练前练后快速查阅，**把散乱的知识点收成一张可浏览的地图**。

### 🏆 游戏化激励 · 连续打卡与链上里程碑
连续打卡、累计训练次数在本地记录并驱动 streak 与成就；**首次参赛、首次拿名次、首次夺冠、升带** 等关键节点可在「训练里程碑」领取 **Solana Devnet 链上证明**（教练钱包签名验证）。不可篡改，永远属于你。

### 🧠 AI 教练 · 个性化方案 + 夸夸
基于年龄、腰带、目标、当日感受，生成专属训练方案（热身 → 技术 → 体能 → 放松，含练后总结）。点击「太难 / 太简单」动态调整强度。**每次附赠 3 个 AI 夸夸**（从你的数据里发现光；API 不可用时也有规则版夸夸兜底）。

**使用流程**：首页进入「AI 教练方案」→ 先填写并保存个人资料 → 点击「生成专属训练方案」后才会调用 API 生成。

### 🔐 钱包 / 邮箱登录 · 链上认证
支持 **OKX Wallet**、**Phantom Wallet** 或 **邮箱** 登录。升带与重要里程碑需教练钱包签名验证后上链，**形成不可磨灭的柔术履历**。

### 🌐 多语言
界面与教练文案支持 **简体中文、繁体中文、英文、日文**，一键切换。
...
| **混合架构 + 兜底** | LLM 失败时自动降级到纯规则版（含夸夸），永不空手而归 |

### Web3 原生

- **钱包 / 邮箱登录**：OKX、Phantom 或邮箱身份
- **链上里程碑**：比赛与升带等关键成就通过教练签名上链（Solana Devnet）
- **用户数据主权**：训练日志本地存储（LocalStorage）；里程碑证明可选上链存证
...
**生产环境**：https://phantom-thief-s-mat-main.vercel.app

推送 `main` 到 GitHub 可触发 Vercel 自动部署。手动部署请始终使用项目 **`phantom-thief-s-mat-main`**（不要用单独的 `rebelbjj` 别名）：
